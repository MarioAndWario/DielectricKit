...
module epswrite_hdf5_m
  ...
  implicit none
  private
  public :: eps_hdf5_setup, set_qpt_done, is_qpt_done, eps_hdf5_setup_2, eps_hdf5_setup_3
contains
  subroutine eps_hdf5_setup(kp, gvec, syms, crys, pol, qgrid, nq, qpts, nmtx, nmtx_max, nband, name, restart)
    ...
    if(pol%subspace  .and. (.not. pol%use_hdf5) ) then
       ...
       call hdf5_write_logical(file_id, 'eps_header/subspace/write_inveps_in_subspace', pol%matrix_in_subspace_basis, error)
       if (error .ne. 0) then
          call die("HDF5 error", only_root_writes=.true.)
       endif
       ...
       call hdf5_create_dset(file_id, 'eps_header/subspace/neig', H5T_NATIVE_INTEGER,(/nq/), error)
       if (error .ne. 0) then
          call die("HDF5 error", only_root_writes=.true.)
       endif
  ...
  subroutine eps_hdf5_setup_2(kp, gvec, syms, crys, pol, name, restart)
    type(kpoints), intent(in) :: kp
    type(gspace), intent(in) :: gvec
    type(symmetry), intent(in) :: syms
    type(crystal), intent(in) :: crys
  ...
  end subroutine eps_hdf5_setup_2

  subroutine eps_hdf5_setup_3(kp, gvec, syms, crys, pol, name, iq_start, iq_end, restart)
    type(kpoints), intent(in) :: kp
    type(gspace), intent(in) :: gvec
    type(symmetry), intent(in) :: syms
    type(crystal), intent(in) :: crys
  ...
  end subroutine eps_hdf5_setup_3  
#endif
