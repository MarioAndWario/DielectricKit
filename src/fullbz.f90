...
! Module:
!
! (1) fullbz      by Meng Wu (2020)
!
!
!     Given reduced kpoints in Brillouin zone (BZ), return full kpoints
!     in Wigner-Seitz(WS) BZ
!
!
!--------------------------------------------------------------------
  ...
  subroutine fullbz(crys, syms, gr, use_identity_only, nfix)
    ...
    do irk = 1, gr%nr
       ...
       if (NORM2(rk_temp(:) - gr%r(:,irk)) > TOL_ZERO) then
          !> call die("fullbz: Please use RBZ kpoints in WS BZ.", only_root_writes=.true.)
