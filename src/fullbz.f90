...
module fullbz_m
  ...
  implicit none
  private
  public :: fullbz_2, dealloc_grid, dealloc_grid_simple, bz_r2f_copy
contains
  ...
  !> We use WS BZ in fullbz_2
  subroutine fullbz_2(crys,syms,gr,use_identity_only,nfix)
    ...
    do irk = 1, gr%nr
       ...
       if (NORM2(rk_temp(:) - gr%r(:,irk)) > TOL_ZERO) then
          !> call die("fullbz_2: Please use RBZ kpoints in WS BZ.", only_root_writes=.true.)
