...
subroutine genwf_chi(pol, ik, iq, crys, gvec, kg, kgq, syms, symsq, wfnc, wfnv, intwfnc, intwfnvq)
  ...
  SCALAR, allocatable :: cg_(:,:,:), cg_2d(:,:), cg_2d_(:,:) !> cg_(ig, ib, is)
  ...
  if (iq <= pol%nq0) then
     ...
     call gmap_2(gvec, symsq, wfnv%ng, kgq%itran(pol%indexq(ik)), kgq%kg0(:,pol%indexq(ik)), isort_new2gvec, isorti_gvec2old, ind, ph)

     !> intwfnvq%cgk(ig, ib, is, irk)
     cg_(1:wfnv%ng, 1:wfnv%nband, 1:wfnv%nspin*wfnv%nspinor) = intwfnvq%cgk(1:wfnv%ng, 1:wfnv%nband, 1:wfnv%nspin*wfnv%nspinor, irkq_loc)
  ...
  ind = 0
  ph  = ZERO
  call gmap_2(gvec, syms, wfnc%ng, kg%itran(ik), kg%kg0(:,ik), isort_new2gvec, isorti_gvec2old, ind, ph)

  cg_(1:wfnc%ng, 1:wfnc%nband, 1:wfnc%nspin*wfnc%nspinor) = intwfnc%cgk(1:wfnc%ng, 1:wfnc%nband, 1:wfnc%nspin*wfnc%nspinor, irk_loc)
